...
public class TulipConfigManager {
    ...
    public void save() {
        try {
            ...
            Bring_Decay.LOGGER1.error("Could not save Tulip config file for mod " + modid, ioException);
        }
    }
    ...
    public void saveUnsafe() throws IOException {
        Bring_Decay.LOGGER1.info("Saving Tulip config to path '" + configPath + "'...");

        if (!fileExists()) {
            Bring_Decay.LOGGER1.warn("Tulip config for mod " + modid + " does not exist. Creating a new config file, but the values may be wrong");
        }
    ...
    public void load() {
        try {
            ...
            Bring_Decay.LOGGER1.error("Could not load Tulip config file for mod " + modid, ioException);
        }
    }
    ...
    public void loadUnsafe() throws IOException {
        Bring_Decay.LOGGER1.info("Loading Tulip config from path '" + configPath + "'...");

        if (!fileExists()) {
            Bring_Decay.LOGGER1.error("Tulip config for mod " + modid + " does not exist. Tulip will create a new config file, but this error should be reported.");
            createFileWithProperties();
        }
    ...
    public void createFileWithProperties() throws IOException {
